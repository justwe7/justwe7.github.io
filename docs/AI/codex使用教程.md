...
**执行中打字输入：排队和引导是两个独立行为**

两者都支持"不杀死当前动作、事后再处理"这种打断方式，但 Codex 把它拆成了两个可选行为，Claude Code 只有一个：

| 操作 | Codex CLI | Claude Code |
|------|-----------|-------------|
| 排队（不影响当前轮） | `Tab` 提交，等**整轮**任务结束后才处理 | 无此选项 |
| 引导（影响当前轮） | `Enter` 提交，直接注入当前轮，Codex 据此调整 | 打字 + `Enter`，等**当前这一步工具调用**结束后读取并调整 |
| 硬中断 | `Esc`——若有排队消息会强制立刻处理，否则取消当前轮 | `Esc`——取消当前动作，等待全新指令 |
| 默认行为 | 有设置项可选 `Enter` 默认走排队还是引导 | 固定为引导，无需配置 |

核心差异在于粒度：Codex 的"排队"要等**整个 turn** 跑完，"引导"和 Claude Code 的默认行为大致等价；Claude Code 没有区分排队和引导，打字追加统一在**当前工具调用**结束的边界上生效，粒度更细，但也少了"完全不打扰本轮，攒到下一轮再看"的选项。

> Codex 的这套排队/引导机制是 2026 年初才上线的新功能，官方 GitHub 上还有不少 issue 反馈 `Tab`/`Enter` 的映射存在回归和 bug，实际行为可能随版本变化，以当前安装版本的真实表现为准。
