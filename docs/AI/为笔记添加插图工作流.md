...
这篇笔记记录一次把“文章配图”流程接入当前 Docusaurus 知识库的过程。目标是让后续配图可以稳定复用：用 Skill 生成图片，用项目规则约束保存路径，用 WebP 压缩控制体积，最后在 Markdown 中引用压缩后的图片。

---

## 一、启用文章配图 Skill

当前项目使用 Codex 辅助整理笔记和生成配图。为了让 Codex 在项目内识别文章配图工作流，需要把本地维护的 `article-metaphor-illustrator` Skill 软链接到项目的 `.agents/skills/` 目录。
