...
Claude 执行到一半时，直接打字并回车，**不会**打断当前正在跑的工具调用——Claude 会等这次调用结束后再读取这段文字，并据此调整下一步动作，之前的进度不会作废。这和 `Esc` 完全不同：`Esc` 是直接取消当前动作，等待一条全新指令。方向大致对、只是想临时补充或纠偏时，用打字追加；发现整个方向跑偏了，才用 `Esc` 整个打断。
